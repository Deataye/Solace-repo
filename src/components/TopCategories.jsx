  ...
  return (
    <div className="bg-[#D8E0E9] py-16">
      <div className="container mx-auto px-4">
        {/* Section Title */}
        <h2 className="text-[#1C4166] text-[55px] font-gidugu font-normal mb-10 text-start">
          Top Categories
        </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 justify-items-center">
          {categories.slice(0, 2).map((item) => (
            <div
              key={item.id}
              className="relative rounded-3xl lg:w-[600px] w-[400px] h-[270px] lg:h-[270px] overflow-hidden"
            >
              {/* Background Image */}
              <img
                src={item.image}
              ...
              <div className="absolute left-0 top-0 h-full w-[200px] bg-[#D9D9D973] flex flex-col justify-between p-8">
                <div className="w-[130px]">
                  <h3 className="text-[18px] font-gidugu font-normal leading-[33.37px] text-white mb-4">
                    {item.title}
                  </h3>
                  <p className="text-[8px] font-inter font-thin leading-[9.68px] text-gray-700">
                    {item.description}
                  </p>
                </div>
                <button className="self-start px-4 py-2 rounded-full text-[8px] bg-[#1C4166] text-yellow-500 hover:opacity-90 transition-opacity">
                  Shop now
                </button>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-8">
          {categories.slice(2).map((category) => (
            ...
            >
              {/* Background Image */}
              <div
                className="h-[413px] bg-cover bg-center"
                style={{ backgroundImage: `url(${category.image})` }}
              ></div>

              {/* Content */}
              <div className="absolute bottom-0 left-0 right-0 bg-[#D9D9D973] p-6 grid grid-cols-2 items-center text-white">
                <div>
                  <h3 className="text-[18px] font-gidugu font-normal mb-2">
                    {category.title}
                  </h3>
                  <p className="text-[12px] font-inter font-light leading-[18px] mb-4">
                    {category.description}
                  </p>
                </div>
                <div className="flex justify-center">
                  <button className="bg-[#1C4166] text-yellow-500 text-[12px] py-2 px-4 rounded hover:bg-[#142b4a]">
                    Shop now
                  </button>
                </div>
              </div>
        ...
        <div className="mt-8 text-center">
          <button className="text-[#1C4166] text-lg underline hover:text-[#142b4a]">
            Show more
          </button>
        </div>
      </div>
